...
const PustakaMakna = () => {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("pustakaMaknaData"));
    if (storedData) {
      setData(storedData);
  ...
  useEffect(() => {
    if (data.length > 0) {
      localStorage.setItem("pustakaMaknaData", JSON.stringify(data));
    }
  }, [data]);
  ...
  const handleUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      // Memecah file .txt menjadi baris-baris dan mengonversinya menjadi objek data
  ...
  // Fungsi untuk menyimpan data baru atau mengedit data yang ada
